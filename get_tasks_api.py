...
while True:
    ...
    try:
        ...
        for r in rs:
            try:
                task_id = r['id'] # int
                if (r['data_request'] != None and not database.check_task_exists(task_id)):
                    ...
                    database.add_task(task_id, type_task, owner_id, item_id, count, payInfo)
                    print(f'Задача добавлена task_id: {task_id}, count: {count}, type_task: {type_task}, owner_id: {owner_id}, payInfo: {payInfo}')
                else:
                    print("data NONE")
                    pass
